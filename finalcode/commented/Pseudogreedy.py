# Pseudo-greedy board generation
# Programmeertheorie
# Case: Amstelhaege
#
...
def min_val():
    # determins the minimum value of all houses on the board
    x = houses
    ...
    # retrieves the amount of each type of homes
    Small = x['familyhomes']
    Mid = x['bungalows']
    Big = x['maisons']
...
def hill_climb(allhomesinfo, oldvalue):
    # hill_climber to try and approach the local maximum
    ...
    # itterates 2000, making changes along the way.
    for i in range(2000):
        ...
        # checks if the generated value is larger than the maximum width
        if j1 > MAXWIDTH or j2 > MAXWIDTH:
            j = random.randint(0, MAXWIDTH)
            j1 = j
            j2 = j1 + housingtype["width"]
        # checks if the maximum value is larger than the maximum length
        if k1 > MAXLENGTH or k2 > MAXLENGTH:
            ...
            print(oldvalue)
            print(newvalue)
            # checks if the new value is larger than the old value
            if newvalue > oldvalue:
                oldvalue = newvalue
                # if so generates a new random variable j, k, h
                j = random.randint(0, MAXWIDTH)
                k = random.randint(0, MAXLENGTH)
                h = random.randint(0, x-1)
...
def value():
    ...
    housingdict = []
    counter = 0
    # itterates of allhomesinfo, a list of dicts to obtain x and y coordinates
    # and determins the free space of each home
    for i in allhomesinfo:
        ...
        j = 0
        # itterates over a newly created dict and determins the value of every home
        # summing it up
        for i in distanceallhouse:
            sumofvalue += ((i["f"] * housingdict[j]["type"]["value increase"])) + i["Value"]
            j += 1
    # returns the value of all homes on the grid
    return sumofvalue


def watershape():
    # generates 1 to 4 bodies of water with rations of 1:1 to 1:4

    # gets max area of the array
    maxarea = MAXLENGTH * MAXWIDTH
    # water surface area of the total grid
    wsa = 0.2 * maxarea
    # minimum width of the board
    minwidth = 1
    while True:
        # average water tile, determins how many water tiles should exist
        awt = wsa / random.randint(1,4)
        # gets the random interger back for later
        number = int(wsa / awt)
        # generates a random width between 1/50th and 1/xth(max=4) of the width
        width = random.randint(minwidth, int(MAXWIDTH / number))
        length = int(awt / width)
        # if the length matches the requirements, gets out of the loop
        if int(width / length) >= 1 and int(width / length) <= 4 or int(length / width) >= 1 and int(length / width) <= 5:
            break
    # returns a dict with the amount of water tiles, the width and the length
...
def houseamount():
    # generates a dict with the amount of each home
...
def start_generator():
    # clears the board and starts to generate a configuration
...
def main():
    ...
    global minimum
    minimum = min_val()
    # gets and sets info pertaining the houses
