...
def hill_climb(allhomesinfo):
    ...
    for i in range(100):
        ...
        if k1 > MAXLENGTH or k2 > MAXLENGTH:
            ...
            info = {}
            np.place(board[y1:y2,x1:x2], board[y1:y2,x1:x2] == int(housingtype["type"]), 0)
            #allhomes.remove(oude entry)
            np.place(board[k1:k2,j1:j2], board[k1:k2,j1:j2] == 0, int(housingtype["type"]))
        ...
        if newvalue > oldvalue:
            ...
            np.place(board[k1:k2,j1:j2], board[k1:k2,j1:j2] == int(housingtype["type"]), 0)
            np.place(board[y1:y2,x1:x2], board[y1:y2,x1:x2] == 0, int(housingtype["type"]))
