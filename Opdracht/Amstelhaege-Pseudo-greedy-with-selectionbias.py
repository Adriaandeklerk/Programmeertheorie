...
def score(repititions):
    ...
    for i in range(rep):
        print "Iteration: ", i
        start_generator()
        #AB = hill_climbing() # -> is to return the free space atleast, so the value function
    ...
    return {"best_conf":best_conf, "best_val":best_val}
...
def image(board):
    ...
    img.set_cmap('Accent')
    PLT.axis('off')
    PLT.savefig('greedymap.png', bbox_inches='tight')
    PLT.show()
...
def main():
    ...
    # shows the board
    scorepoints = score(repititions)
    board = scorepoints["best_conf"]
    best_val = scorepoints["best_val"]
    print "highest value is:",best_val
